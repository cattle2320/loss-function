...
def precision(confusion):
    denominator = np.diag(confusion)
    molecule = np.diag(confusion) + confusion.sum(axis=0)
    result = denominator / (molecule + 1e-10)
    return result

def recall(confusion):
    denominator = np.diag(confusion)
    molecule = np.diag(confusion) + confusion.sum(axis=1)
    result = denominator / (molecule + 1e-10)
    return result

def dice(confusion):
    denominator = 2 * np.diag(confusion)
    molecule = (confusion.sum(axis=1) + confusion.sum(axis=0))
    result = denominator / (molecule + 1e-10)
    return result
